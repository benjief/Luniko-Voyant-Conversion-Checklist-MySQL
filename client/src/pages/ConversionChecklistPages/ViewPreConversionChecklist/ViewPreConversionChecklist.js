...
function ViewPreConversionChecklist() {
    ...
    // Unchanging variables should be decalred as plain old constants
    const loadErrorMessage = "Apologies! We've encountered an error. Please attempt to re-load this page.";
    const writeErrorMessage = "Apologies! We've encountered an error. Please attempt to update your checklist again.";
    ...
    const checkLoadSheetNameEntered = () => {
        for (let i = 0; i < validLoadSheetNames.current.length; i++) {
            let escapeRegExpMatch = formProps["loadSheetName"].replace(/[-/\\^$*+?.()|[\]{}]/g, '\\$&'); // TODO: check this to make sure it works (got rid of escape character)
